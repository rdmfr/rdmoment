...
const features = [
  {
    ...
    description:
      'Edit warna, font, dan layout secara real-time dengan editor visual yang intuitif.',
  },
  {
    icon: Heart,
    title: 'Tema Budaya',
    description:
      'Pilih dari 50+ tema yang terinspirasi dari tradisi budaya di seluruh dunia.',
  },
  {
    icon: Users,
    title: 'Buku Tamu Interaktif',
    description:
      'Tamu dapat meninggalkan ucapan dan pesan langsung di undangan Anda.',
  },
  {
    icon: Music,
    title: 'Musik & Media',
    description:
      'Tambahkan lagu favorit, foto, dan video untuk personalisasi undangan.',
  },
  {
    icon: Gift,
    title: 'Daftar Hadiah',
    description:
      'Integrasikan gift registry dan kelola RSVP dalam satu tempat.',
  },
  {
    icon: Share2,
    title: 'Mudah Dibagikan',
    description:
      'Bagikan via link, QR code, atau media sosial dengan satu klik.',
  },
];
  ...
  return (
    <section id="features" className="py-24 px-6 sm:px-8 lg:px-12 bg-zinc-950">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <AnimateOnScroll animationClassName="animate-slide-up-fade" className="text-center mb-20">
          ...
          <p className="text-base md:text-lg lg:text-xl text-amber-100/60 max-w-3xl mx-auto font-light leading-relaxed">
            Fitur-fitur powerful yang dirancang untuk membuat undangan
            pernikahan Anda tak terlupakan.
          </p>
        </AnimateOnScroll>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <AnimateOnScroll
                key={index}
                animationClassName="animate-slide-up-fade"
                delay={index * 100}
              >
                <div className="group relative h-full p-8 rounded-2xl border-2 border-amber-900/20 bg-gradient-to-br from-zinc-900 via-black to-zinc-900 hover:border-amber-500/40 transition-all duration-500 hover:shadow-xl hover:shadow-amber-500/10">
                  {/* Icon Container */}
                  <div className="relative mb-6">
                    <div className="inline-flex p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 group-hover:bg-amber-500/20 group-hover:border-amber-500/30 transition-all duration-500">
                      <Icon
                        className="w-8 h-8 text-amber-500 group-hover:scale-110 transition-transform duration-500"
                        strokeWidth={1.5}
                      />
                    </div>
